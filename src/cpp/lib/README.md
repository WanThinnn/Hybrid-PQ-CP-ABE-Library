...
go build -buildmode=c-shared -o libcpabe_tkn20.dll cpabe_tkn20.go
```
> **Note for Windows (MSVC) users:** The static library (`.lib`) generated by Go on Windows (which relies on MinGW/GCC) is often incompatible with the MSVC linker due to ABI differences. It is highly recommended to rely on the shared library (`.dll`) when compiling the main project with MSVC.

**Linux:**
```bash
cd include/tkn20
go build -buildmode=c-archive -o libcpabe_tkn20.a cpabe_tkn20.go
go build -buildmode=c-shared -o libcpabe_tkn20.so cpabe_tkn20.go
```
This will generate the static/shared libraries and the C header file (`libcpabe_tkn20.h`). After building, you must move the generated library files to their respective directories in the project:
- Move `libcpabe_tkn20.lib` (or `.a`) to the `lib/static/` directory.
